...
{
    public class ColumnToObject
    {
        readonly ColumnsNames Names;
        readonly InstanceModel Model;
        readonly ReadOnlyCollection<TableName> Tables;

        public ColumnToObject(ColumnsNames names, InstanceModel model, IList<TableName> tables)
        {
            Names = names;
            Model = model;
        ...
        public ColumnToObjectResponse SetColumnToObject<TModel>(ColumnValue value, DataRow reader, TModel item, string actualTable)
        {
            ...
            else
            {
                for (int index = 0; index < c; index++)
                {
                    if (columns[index].Options is not null)
                    {
                        if (!columns[index].Options.Ignore)
                        {
                            value.SetValue(columns[index], reader);
                        }
                    }
                    else
                    {
